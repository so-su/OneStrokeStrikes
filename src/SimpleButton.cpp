...
SimpleButton::SimpleButton(const Rect& rect_, Color inner_color_,
                           Color gauge_color_, int32 thickness_)
    : rect(rect_),
      top{rect.top()},
      right{rect.right()},
      bottom{rect.bottom()},
...
bool SimpleButton::update(bool can_press) {
    ...
    gauge_transition.update(pressed or
                            (rect.contains(Cursor::Pos()) and can_press));
...
// ボタン内部の色を変更する
void SimpleButton::set_inner_color(Color color) { inner_color = color; }
