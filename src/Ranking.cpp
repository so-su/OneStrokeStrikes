...
Ranking::Ranking(const InitData& init) : IScene{init}{
    ...
    if(getData().display_player_score){
        display_register_button=(std::size(ranking)<10) or (getData().score > ranking.back().score);
...
void Ranking::draw() const{
    ...
    if(display_register_button){
        ...
        FontAsset(U"Regular")(U"ランキング登録").drawAt(20,ranking_register.center(),Palette::Black);
    }
    ...
    if(input_mode){
        Scene::Rect().draw(ColorF{Palette::Black,mask_alpha_transition.value()*0.6});
        round_rect.draw(ColorF{0.9});
        FontAsset(U"Regular")(U"ニックネームを入力してください").drawAt(700,310,Palette::Black);
        FontAsset(U"Regular")(U"3文字以上16文字以下で、半角英数字, -, _ が使えます。\nまた、アルファベットから始まる必要があります。").drawAt(15,700,360,Palette::Dimgray);
        ...
        send.draw();
        FontAsset(U"Regular")(U"登録").drawAt(20,send.center(),Palette::Black);
        
        input_backward.draw();
        FontAsset(U"Regular")(U"もどる").drawAt(20,input_backward.center(),Palette::Black);
    }
}
...
bool Ranking::send_score()const{
    const std::string data = (U"user_id={}&score={}"_fmt(text_edit.text,getData().score)).toUTF8();
...
void Ranking::draw_ranking()const{    
    for(auto rank:step(10)){
        ...
        FontAsset(U"Kaisotai")(rank+1).draw(40,Arg::rightCenter=Vec2{450,170+50*rank+25},Palette::Black);
    }
    
    for(auto rank:step(std::size(ranking))){
        const auto user=ranking[rank];
        FontAsset(U"Regular")(user.user_id).draw(Arg::leftCenter=Vec2{500,170+50*rank+25},Palette::Black);
        FontAsset(U"Regular")(user.score).draw(Arg::rightCenter=Vec2{980,170+50*rank+25},Palette::Black);
    }
    
    if(player_place.has_value()){
        Triangle{370,static_cast<double>(195+50*player_place.value()),30,90_deg}.moveBy(7*Math::Sin(5*Scene::Time()),0).draw(Palette::Black);
    }
    
    if(getData().display_player_score){
        rects[10].drawFrame(1,1,Palette::Dimgray);
        FontAsset(U"Regular")(U"あなたのスコア").draw(Arg::leftCenter=Vec2{500,170+50*10+25},Palette::Black);
        FontAsset(U"Regular")(getData().score).draw(Arg::rightCenter=Vec2{980,170+50*10+25},Palette::Black);
    }
}

bool Ranking::is_valid(const String& user_id){
    return std::size(user_id)>=3 and IsAlpha(user_id.front()) and user_id.all([](char32 c)->bool{return IsAlnum(c) or c=='-' or c=='_';});
}
