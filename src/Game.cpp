...
void Game::update() {
    ...
    if (attack_mode) {
        ...
        if (time < 1.0) {
            ...
            double roulette_speed =
                20.0 * (1.0 - attack_mode_timer.sF() / roulette_duration);
...
void Game::draw() const {
    ...
    for (auto enemy_idx : step(3)) {
        ...
        else if (vanishing_timers[enemy_idx] <= 0.49 and
                 respawn_timers[enemy_idx] < 0.5) {
            enemy.draw_path();
        }
    }
    ...
    if (attack_shape != nullptr) {
        ...
        Point center =
            (pos - upper_left) / 30 * 30 + upper_left + Point{15, 15};
        attack_shape->draw(
