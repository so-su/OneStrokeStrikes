...
Button::Button(const Array<Point>& path_, Size grid_size_, int32 cell_size_,Point upper_left,Color color_):path(path_),rects(grid_size_.y,grid_size_.x,none),grid_size(grid_size_),cell_size(cell_size_),color(color_),mouseover_transition{0.2s,0.1s},pressed_transition{0.8s,0.4s},alpha_transitions(std::size(path),Transition{0.1s,0.05s}){
    for(const auto& cell_pos:path){
        rects[cell_pos.x][cell_pos.y]=Rect{upper_left+cell_size*cell_pos,cell_size,cell_size};
    }
    ...
    do{
        for(int32 d=1;d<=4;++d){
            ...
            if(graph[now.x][now.y][next].has_value()){
                perimeter.emplace_back(upper_left+now*cell_size,upper_left+*graph[now.x][now.y][next]*cell_size);
...
void Button::draw()const{
    for(auto cell_idx:step(std::size(path))){
        ...
        rects[x][y]->draw(ColorF{color,0.7+0.3*alpha_transitions[cell_idx].value()});
    }
}
...
void Button::draw_gauge()const{
    ...
    if(len_int<std::size(perimeter)){
        perimeter[len_int].stretched(0,cell_size*((gauge_len-len_int)-1.0)).draw(12,Palette::Black);
    }
    ...
    if(len_int<std::size(perimeter)){
        perimeter[len_int].stretched(0,cell_size*((gauge_len-len_int)-1.0)).draw(10,MyColor::Orange);
    }
}
...
void Button::update(){
    ...
    for(auto dist:step(std::size(path))){
        ...
        alpha_transitions[cell_idx].update(pressed_transition.value()>=(dist+1)/path_len);
    }
}
